...
class SpeedRpmGauge(FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.gear_value = self._make_label("D", dp(64), (0.56, 0.96, 0.47, 1), size=(dp(110), dp(84)), bold=True, italic=True)
        self.unit_label = self._make_label("KM/H", dp(28), (0.78, 0.79, 0.81, 1), size=(dp(140), dp(38)), bold=True, italic=True)
    ...
    def redraw(self, *args):
        ...
        with self.canvas:
            ...
            Line(points=self._arc_points(cx, cy, radius, self.start_angle, self.end_angle, 120), width=dp(3))

            Color(0.93, 0.95, 0.98, 0.65)
            Line(points=self._arc_points(cx, cy, radius * 0.14, 156, 18, 40), width=dp(2))
            ...
            Color(0.47, 0.48, 0.51, 0.9)
            Line(points=self._arc_points(cx, cy, inner_face_radius * 1.08, 154, 24, 50), width=dp(2))

            Color(0.47, 0.97, 0.49, 1)
            Line(circle=(cx, cy, inner_face_radius * 0.68), width=dp(2.3))
    ...
    def _draw_needle(self, cx, cy, length, ratio):
        ...
        Line(points=[tail_x, tail_y, tip_x, tip_y], width=dp(5.2), cap="round")

        Color(0.58, 0.58, 0.6, 1)
        Ellipse(pos=(cx - dp(12), cy - dp(12)), size=(dp(24), dp(24)))

        Color(0.17, 0.18, 0.21, 1)
